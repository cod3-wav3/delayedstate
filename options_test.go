...
func TestWithOnStateChangeOptionSetsCallback(t *testing.T) {
	...
	sc := NewStateController(WithOnStateChange(mockCallback))
...
func TestOnStateNotExistCreatesState(t *testing.T) {
	mockCallback := func(name string) (State, error) {
		return State{Delay: time.Millisecond * 5, DelayOnActivation: true}, nil
	}
	...
	if !state.State.DelayOnActivation {
		t.Fatal("Expected state to have DelayOnActivation=true")
	}
